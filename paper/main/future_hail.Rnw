...
\abstract{Hailstorms cause damage across the globe and pose significant risk to
crops, but changes to hailstorms in a warming climate are not well quantified.
We applied three hail proxies to an ensemble of global model projections to
quantify changes in the frequency of hail-prone conditions worldwide. Changes
were divergent, depending on proxy treatment of temperature. Uncertainty on hail
projections remains high, especially in the tropics. However, in projections
with 2 \degr{} and 3 \degr{} of mean global warming, ensemble-mean hail-prone
conditions shifted poleward, with decreases in hail hazard across the
mid-latitudes and increases in colder regions. We calculated the resulting
effects for 26 crop types. With fixed exposure and vulnerability, hail risk was
generally projected to increase for winter crops such as wheat and decrease for
summer crops such as maize. Poleward shifts in hail hazard may attenuate any
positive impacts of similar shifts in crop regions under climate change.}
...
Hailstorms can cause significant damages to crops by breaking stems,
defoliating, or crushing plants \cite{Dhillon_CJPS_2021}. Hail can also wound
plants and lead to increased pathogen infection \cite{Battaglia_CS_2019}. Hail
can thus result in significant losses of crop yield \cite{Dhillon_CJPS_2021}.
Factors that affect the amount of crop damage inflicted by hail include the
hailstone size \cite{Changnon_JAMC_1971} and number of hailstones per area
...
While correlations between temperature and hailstorm damage in Europe
\cite{Dessens_GRL_1995} have been used to project regional increases in damage
with warming \cite{Botzen_REE_2010}, no global picture on possible
climate-induced changes to hail hazard for crops has emerged.
...
\begin{figure*}[!ht]
    ...
    multimodel mean historical hail-prone days over land, for 3\degr{} warming
    for Asia (a), Australasia (b), North America (c), South America (d), Europe
    (e), and Africa (f). Stippling as for Figure
...
\begin{figure*}[!ht]
    ...
    calculated using a well-mixed parcel on the lowest 100 hPa, e-h),
    most-unstable mixing ratio (MUMR, i-l), and temperature at 500 hPa (T500,
    m-p). ``Sum'' (q-t) shows sums across changes from all de-biased ingredients
    (column sums from Supplementary Figure \ref{SM-fig:drivers}) and ``Ref''
    (u-x) shows the projected changes per proxy when no ingredients were
...
to reduce, severity may increase. However, it remains difficult to draw broad
...
large hail aligns with hail size end-of-century projections for the USA
\cite{Gensini_npjCAS_2024}. Pseudo-global warming simulations for hailstorms in
the USA showed a seasonal shift with increasing hailstone sizes in cold-season
storms \cite{Mallinson_JC_2024}. While it is likely that global severe hail
projections would differ significantly from the ``any hail'' projections shown
here, the possibility of some areas experiencing compounding risks with
increased frequency (shown here) combined with increased hail stone size
\cite{Mallinson_JC_2024} may produce a pronounced risk to winter crops that
...
1$^{\circ}$$\times$1$^{\circ}$ resolution. Cropping periods for each
1$^{\circ}$$\times$1$^{\circ}$ grid point were calculated as the union of all
cropping periods, per crop, from MIRCA2000 \citep{Portmann_GBC_2010} data at 0.5$^{\circ}$$\times$0.5$^{\circ}$ resolution. MIRCA2000 provides global cropping periods for 26 crops for the year 2000.
...
in Supplementary Material Section \ref{SM-sec:proxy-update} and the formulas for
the Eccel and SHIP proxies are shown in Supplementary Materials Section
\ref{SM-sec:proxy-formulas}. Other proxies were tested but excluded from this
study because they produced unrealistically many or too few hail-prone days in
comparison with the other proxies (Supplementary Figure
\ref{SM-fig:historical_all_proxies}). The Raupach proxy without extra conditions
produced similar results to the Raupach proxy with extra conditions
