...
hailstorms in a warming climate are not well quantified. Here, we applied three
hail proxies to an ensemble of eight global model projections to quantify
changes in the frequency of hail-prone conditions worldwide. Proxy changes were
divergent, with results depending on proxy treatment of temperature.
Accordingly, uncertainty on hail projections remains high, especially in the
tropics. However, in projections with 2 \degr{} and 3 \degr{} of mean global
warming, ensemble-mean hail-prone conditions shifted poleward, with decreases in
hail hazard frequency across the mid-latitudes and increases in colder regions.
As a sensitivity study we calculated the effects of those changes for 26 crop
types. In general, hail-prone frequency was projected to decrease in summer and
increase in winter. Accordingly, if exposure and vulnerability are held fixed,
hail risk was projected to increase for winter crops and decrease for summer
crops.} 
...
\cite{Raupach_MWR_2023}. Proxies suffer, however, from the ``initiation
problem'', in that storms rarely initiate even in storm-prone conditions
\cite{Sherwood_MWR_1999, Tippett_CCCR_2015, Raupach_MWR_2023}.
...
heterogeneity in trends in hail frequency, owing in part to offsetting in
...
Here, we applied three hail proxies to an ensemble of eight global projections
from CMIP6, to examine projections of the frequency of hail-prone conditions
globally. We used a per-degree framework \cite{Lepore_EF_2021} for simpler
comparisons between models. As a sensitivity study, we investigated the effects
of projected changes on hail-prone proportions of time-inveriant crop growing
seasons for 26 different crops. For details of the simulations and proxies used,
refer to ``Online methods'' and the supplementary information. Our work shows
that, generally, hailstorm frequency is projected to shift poleward, with
decreases in warmer environments but increases in cooler environments in future,
driven primarily by changes in convective instability. Our sensitivity analysis
shows that the projected changes would increase hail occurrence risk to winter
crops, while decreasing occurrence risk for summer crops.
...
which is especially important for smaller hail \cite{Pruppacher_2010}.
...
previous results, by region. In Asia, our results match past decreases in hail
occurrence found in stations concentrated in China's east from 1980-2012
\cite{Xie_GRL_2008, Li_JC_2016, Zhang_SR_2017}, but contradict past decreases in
...
\cite{Liu_ERL_2024}. Other studies on past trends in the United States found no
significant trends \cite{Allen_EJSSM_2015, Allen_JAMES_2015, Barrett_MWR_2015}.
Regional projections for North America comparing 1971-2000 to 2041-2070 have
shown decreases in frequency overall and especially in the east and southeast
\cite{Brimelow_NCC_2017} and a near elimination of surface hail in Colorado
\cite{Mahoney_NCC_2012}, agreeing with our projections. Severe hail frequency in
North America is generally projected to increase \cite{Trapp_JC_2019,
Childs_WCS_2020}, although summer decreases in severe hail frequency in the east
are projected \cite{Trapp_JC_2019}. In South America, there are few trend
...
over 1960-2008 \cite{Mezher_AR_2012}, although there is geographical variability
in observed trends in Argentina \cite{Mezher_AR_2012} and on the
Brazil/Argentina border \cite{Beal_AR_2020}.
...
(1949-1993) \cite{Willemse_1995}, northern Switzerland (1980-2014)
\cite{Madonna_AR_2018} and northern and southern Switzerland (1959-2022)
...
\cite{Taszarek_JC_2020, Brooks_AR_2003, Raupach_MWR_2023}. However, the Raupach
proxies are specifically designed to take spatial variability in storm
conditions into account by using the melting level height, and have been shown
to perform reasonably across the Australian continent which contains a wide
variety of storm environments \cite{Raupach_npjCAS_2023}, and the multi-proxy
means were able to identify particularly hail-prone conditions in case studies
in areas in which the proxies used here were not trained. Although we show proxy
results globally, we note the proxies used were trained using land-based reports
...
A limitation to our approach is that the cropping periods we consider are
stationary in time and relatively coarse, so our analysis of crop changes should
be taken as a sensitivity study rather than projections of changes in crop
damage or yield. Suitable growing regions and cropping periods may also shift
with warming \cite{Franke_GCB_2022}. It is also important to note that the
timing of hail events within the broad cropping season strongly influences
possible crop damage \cite{Dhillon_CJPS_2021}. Future analyses should take
projected changes in cropping season and growth stages into account. 
...
tested but excluded from this study. The proxies of Kunz 2007
\cite{Kunz_NHESS_2007} and Mohr and Kunz, 2013 \cite{Mohr_AR_2013} produced
unrealistically many hail-prone days in tropical regions for which they were not
trained \cite{Raupach_MWR_2023}, and a threshold of 0.5 on SHIP, as has been
used in other studies for severe hail \cite{Prein_WCE_2018}, was found to
produce too few hail-prone days in comparison with the other proxies designed
for hail of any size (Supplementary Figure \ref{SM-fig:historical_all_proxies}).
The Raupach proxy without extra conditions produced similar results to the
Raupach proxy with extra conditions (Supplementary Figure
\ref{SM-fig:historical_all_proxies} \todo{and changes}).
