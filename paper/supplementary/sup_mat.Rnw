...
\begin{figure*}[h]
    ...
    (1980-1999), figure shows multi-proxy mean anomalies (n=3 per grid cell).
    The normalisation factor was held constant.}
...
\begin{figure*}[h]
    ...
    \caption{\textbf{Differences in multimodel mean (n=160 per grid point) hail
    ingredients by epoch.} Ingredients shown are annual extreme (99th
    percentile) convective available potential energy (CAPE), annual extreme
    (99th percentile) 0-6 km bulk wind shear (S06), annual extreme (1st
    percentile) convective inhibition (CIN), and annual extreme (1st percentile)
    lifted index. CAPE, CIN, and LI were calculated using a well-mixed parcel
    over the lowest 100 hPa. Stippling shows regions in which at least 50\% of
    the model/proxy combinations agreed with the sign of the mean difference and
...
\begin{table}[h]
    ...
    \begin{tabular}{llll}
        ...
        \bottomrule
    \end{tabular}
    \caption{\textbf{References and licenses for CMIP6 data}.}
...
\begin{figure*}[h]
    ...
    \caption{\textbf{Drivers of changes across models.} As for main text Figure
...
\begin{figure*}[h]
    ...
    \caption{\textbf{Change in multimodel, multi-proxy mean hail-prone
    proportions of cropping seasons for 3 \degr{} warming.} n=480 per grid cell.
    Stippling as for Supplementary Figure \ref{fig:ingredients_changes_1} and
    region subset as for Extended Data Figure
    \ref{ED-fig:historic_crop_proportions}.}
