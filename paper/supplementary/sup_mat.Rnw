...
\begin{figure*}[p]
    ...
    proxy for 2 \degr{} global warming.} Stippling shows regions for which the
...
\begin{figure*}[p]
    ...
    \caption{As for Supplementary Figure \ref{fig:hail_diffs_2C} but for 3
    \degr{} global warming.}
...
\begin{figure*}[p]
    ...
    by season.} DJF stands for December, January, February; MAM stands for
    March, April, May; JJA stands for June, July, August; SON stands for
    September, October, November. Stippling shows regions for which at least
    50\% of the model/proxy combinations showed a statistically  significant
    difference ($p <$ 0.05 using Welch's t-test) and agreed on the sign of the
    change.}
...
\begin{figure*}[p]
    ...
    and annual extreme (1st percentile) lifted index. Stippling as for
    Supplementary Figure \ref{fig:hail_changes_by_season}.}
...
\begin{figure*}[p]
    ...
    proportions of cropping seasons for 2 \degr{} warming.} Stippling as for
    Supplementary Figure \ref{fig:hail_changes_by_season} and region subset as
...
\begin{figure*}[p]
    ...
    is at the centre of the fuschia square in the inset maps. Inset maps show
    \todo{relative} hanges in hail-prone proportion of cropping season. Lines in
    main plot show mean changes in hail-prone days per month over a
    \todo{4$\times$4}\degr{} region around the chosen location (shown as a
    fuschia square in inset maps). Stippling in inset mapas for Supplementary
    Figure \ref{fig:hail_changes_by_season}.}
...
\begin{figure*}[p]
    ...
    \caption{As for Figure \ref{fig:crop_lines_af} but for Canada.}
...
\begin{figure*}[p]
    ...
    with 2 and 3 degrees C warming over the reference period, in SSP5-8.5
