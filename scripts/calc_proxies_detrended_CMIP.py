"""Calculate hail proxy results for detrended CMIP convective data."""
...
assert len(sys.argv) == 2, 'Usage: calc_proxies.py <model_number>'  # noqa: PLR2004
...
dat['surface_height'] = fh.model_orography(dat.attrs['CMIP_model_spec'], match_dat=dat).orog
...
for ing in detrended_ing:
    ...
    if not os.path.exists(outfile_ing):
        ...
        assert not np.any(dat.time.isnull()), 'Time contains missing values.'
        ...
        prox = fh.storm_proxies(dat=detrended, proxies={'proxy_Eccel2012': 'Eccel 2012', 'proxy_SHIP_0.1': 'SHIP > 0.1'})

        # Add Raupach hail proxies.
        print('Raupach proxies...')
        prox['proxy_Raupach2023_updated'] = up.apply_Raupach_proxy(
            dat=detrended,
        ...
        prox['proxy_Raupach2023_updated_noconds'] = up.apply_Raupach_proxy(
            dat=detrended,
            results_file=proxy_results_file,
